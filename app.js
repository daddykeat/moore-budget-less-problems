...
function keyFor(profile){ return `dwr_profile_${profile}`; }
...
function defaultState(){
  return {
    ...
    // Any schedule: paycheck amount + list of pay dates
    income: [
      { id: crypto.randomUUID(), name:"Income", amount:0, payDates:[], avgMode:"annualized" }
    ],
    bills: [
      { id: crypto.randomUUID(), name:"Bill", amount:0, dueDay: 1 }
    ],
    debts: [
      { id: crypto.randomUUID(), name:"Debt", balance:0, apr:0, minPay:0, dueDay: 1, type:"card" }
    ],
    assets: [
      { id: crypto.randomUUID(), name:"Savings", balance:0, monthly:0, type:"savings" }
    ],
    freed: [],
    scoreboard: Array.from({length:12}).map((_,i)=>({
      week: i+1, noNewDebt:false, groceriesOnTarget:false, extraPayment:false
    })),
  };
}
...
function seedWithStarter(s){
  ...
  // migrate older cadence-based income into payDates if possible (keep safe)
...
function loadState(){
  ...
  try { return seedWithStarter(JSON.parse(raw)); }
  catch { return seedWithStarter(defaultState()); }
}
function saveState(){
...
// ----------------- Totals -----------------
function sumBills(bills){ return bills.reduce((a,b)=>a+Number(b.amount||0),0); }
function sumDebtMinimums(debts){ return debts.reduce((a,d)=>a+Number(d.minPay||0),0); }
function sumDebts(debts){ return debts.reduce((a,d)=>a+Number(d.balance||0),0); }
function sumAssets(assets){ return assets.reduce((a,x)=>a+Number(x.balance||0),0); }
function sumAssetContribs(assets){ return assets.reduce((a,x)=>a+Number(x.monthly||0),0); }

function monthlyAverageIncome(income){
  // Uses the next 365 days of payDates to compute monthly average
...
function renderIncome(){
  ...
  state.income.forEach((inc) => {
    ...
    shell.actions.appendChild(mkBtn("Delete", ()=>{
    ...
    const name = textInput(inc.name, v=>{inc.name=v; shell.titleEl.textContent=v||"Income"; saveState(); renderAll();});
    const amt = numInput(inc.amount, v=>{inc.amount=v; saveState(); renderAll();});
    const avgMode = selectInput(inc.avgMode || "annualized", [
      {value:"annualized", label:"Avg monthly from next 365d (recommended)"},
    ], v=>{inc.avgMode=v; saveState(); renderAll();});
    ...
    ta.oninput = ()=>{
      inc.payDates = ta.value
        .split("\n")
        .map(s=>toISODateLoose(s.trim()))
        .filter(Boolean);
...
function renderBills(){
  ...
  state.bills.forEach((b) => {
    ...
    const grid = document.createElement("div");
    grid.className = "grid three";
    grid.appendChild(mkField("Name", textInput(b.name, v=>{b.name=v; shell.titleEl.textContent=v||"Bill"; saveState(); renderAll();})));
    grid.appendChild(mkField("Amount (monthly)", numInput(b.amount, v=>{b.amount=v; saveState(); renderAll();})));
    grid.appendChild(mkField("Due Day (1–28)", dayInput(b.dueDay, v=>{b.dueDay=v; saveState(); renderAll();})));
    shell.wrap.appendChild(grid);
    root.appendChild(shell.wrap);
  });
}
...
function renderDebts(){
  ...
  state.debts.forEach((d) => {
    ...
    const grid = document.createElement("div");
    grid.className = "grid three";
    grid.appendChild(mkField("Name", textInput(d.name, v=>{d.name=v; shell.titleEl.textContent=v||"Debt"; saveState(); renderAll();})));
    grid.appendChild(mkField("Balance", numInput(d.balance, v=>{d.balance=v; saveState(); renderAll();})));
    grid.appendChild(mkField("APR %", numInput(d.apr, v=>{d.apr=v; saveState(); renderAll();})));

    const grid2 = document.createElement("div");
    grid2.className = "grid three";
    grid2.appendChild(mkField("Minimum / month", numInput(d.minPay, v=>{d.minPay=v; saveState(); renderAll();})));
    grid2.appendChild(mkField("Due Day (1–28)", dayInput(d.dueDay, v=>{d.dueDay=v; saveState(); renderAll();})));
    grid2.appendChild(mkField("Type", selectInput(d.type||"card", [
      {value:"card", label:"Credit Card"},
      {value:"loan", label:"Loan"},
      {value:"bnpl", label:"BNPL"},
    ], v=>{d.type=v; saveState(); renderAll();})));
...
function renderAssets(){
  ...
  state.assets.forEach((a) => {
    ...
    const grid = document.createElement("div");
    grid.className = "grid three";
    grid.appendChild(mkField("Name", textInput(a.name, v=>{a.name=v; shell.titleEl.textContent=v||"Asset"; saveState(); renderAll();})));
    grid.appendChild(mkField("Current Balance", numInput(a.balance, v=>{a.balance=v; saveState(); renderAll();})));
    grid.appendChild(mkField("Monthly Contribution", numInput(a.monthly, v=>{a.monthly=v; saveState(); renderAll();})));

    const grid2 = document.createElement("div");
    grid2.className = "grid two";
    grid2.appendChild(mkField("Type", selectInput(a.type||"savings", [
      {value:"savings", label:"Savings"},
      {value:"investment", label:"Investment"},
      {value:"retirement", label:"Retirement"},
      {value:"other", label:"Other"},
    ], v=>{a.type=v; saveState(); renderAll();})));
...
function renderFreed(){
  ...
  state.freed.forEach((f) => {
    ...
    const grid = document.createElement("div");
    grid.className = "grid three";
    grid.appendChild(mkField("Name", textInput(f.name, v=>{f.name=v; shell.titleEl.textContent=v||"Freed Payment"; saveState();})));
    grid.appendChild(mkField("Amount / month", numInput(f.amount, v=>{f.amount=v; saveState(); renderAll();})));
    grid.appendChild(mkField("Month (e.g., 2026-03)", textInput(f.month, v=>{f.month=v; saveState();})));
    shell.wrap.appendChild(grid);
    root.appendChild(shell.wrap);
  });
}
...
function renderScoreboard(){
  ...
  state.scoreboard.forEach((w) => {
    ...
    const mkChk = (label, key) => {
      ...
      i.type = "checkbox";
      i.checked = !!w[key];
      i.onchange = ()=>{w[key]=i.checked; saveState();};
      c.appendChild(i);
...
function applyModes(){
  ...
  // If Strategy tab hidden while active, switch to dashboard
  ...
  debts.forEach((d, idx)=>{
    ...
    row.innerHTML = `
      <div>
        ...
        <div class="meta">${(d.type||"").toUpperCase()} • APR ${Number(d.apr||0).toFixed(2)}% • Min ${money(d.minPay||0)} • Due day ${d.dueDay}</div>
      </div>
...
function build12MonthPreview(){
  ...
  const vars = Number(state.settings.budgets.groceries||0) + Number(state.settings.budgets.gas||0) + Number(state.settings.budgets.other||0);
...
function buildMonthlyEventsWindow(startDateISO, days){
  ...
  // Income events from payDates list
...
function simulateCashflow(){
  ...
  const {start, end, events} = buildMonthlyEventsWindow(startISO, days);
  ...
  for (let i=0; i<=days; i++){
    ...
    const renderGroup = (title, list, colorVar) => {
      ...
      return `
        <div style="margin-bottom:6px;">
          <span class="badge" style="border-color:rgba(93,214,192,.45); color:${colorVar};">${title}</span>
          <div class="cal-items" style="margin-top:6px;">${lines}</div>
        </div>
      `;
    };

    const html = renderGroup("INCOME", inflows, "var(--ok)") +
                 renderGroup("BILLS/PAYS", outflows, "var(--warn)");
    ...
    if (below){
      warnings.push(`Balance drops below safety floor on ${fmtDate(d)} (est. ${money(bal)}).`);
    }
...
function renderDashboard(){
  ...
  const minsEl = $("#mMins"); if (minsEl) minsEl.textContent = money(mins);
  $("#mVars").textContent = money(vars);
  $("#mAttack").textContent = money(attackTotal);

  const nwEl = $("#mNetWorth"); if (nwEl) nwEl.textContent = money(netWorth);
  const nwMeta = $("#mNetWorthMeta"); if (nwMeta) nwMeta.textContent = `${money(assetsTotal)} assets − ${money(debtTotal)} debts`;

  const pDebt = $("#pDebt"); if (pDebt) pDebt.textContent = money(debtTotal);
  $("#pAttack").textContent = money(attackTotal);
  const pSaveInv = $("#pSaveInv"); if (pSaveInv) pSaveInv.textContent = money(investOn ? sumAssetContribs(state.assets) : 0);
  ...
  // Debt thermometer chart: debts vs assets (only meaningful if at least one mode on)
...
function renderHeatmap(){
  ...
  if (!el) return;
  if (!state.settings.modes.debt){ el.innerHTML = ""; return; }
  if (!window.CalHeatmap){ el.innerHTML = `<div class="meta">Heatmap library not loaded.</div>`; return; }
...
// ----------------- Net worth timeline -----------------
function dailyRate(aprPct){ return (Number(aprPct||0)/100) / 365; }
...
function renderNetWorthTimeline(){
  ...
  // shallow debt sim for timeline (simple)
  let working = (debtOn ? state.debts : [])
    .filter(d=>Number(d.balance||0) > 0)
    .map(d=>({ ...d, balance:Number(d.balance||0), apr:Number(d.apr||0), minPay:Number(d.minPay||0), type:d.type||"card" }));
...
function simPayoffWithInterest({ debts, monthlyAttack, strategy, startDate = new Date(), maxMonths = 180 }){
  ...
  if (strategy === "snowball") order.sort((a,b)=>a.balance-b.balance);
    ...
    const rest = order.filter(x=>!(x.type==="bnpl" || x.balance<=1000)).sort((a,b)=>(b.apr-a.apr) || (a.balance-b.balance));
...
function renderSimulation(){
  ...
  sum.innerHTML = `
    ...
    <div class="row"><div class="label">Debt-Free Date (est.)</div><div class="value">${sim.debtFreeDate ? sim.debtFreeDate.toLocaleDateString(undefined,{month:"long",year:"numeric"}) : "—"}</div></div>
  `;

  sim.results.forEach(r=>{
    const monthLabel = r.payoffMonth === null ? "—" :
      addMonths(new Date(), r.payoffMonth).toLocaleDateString(undefined,{month:"short",year:"numeric"});
    const node = document.createElement("div");
    node.className = "item";
    node.innerHTML = `
      <div class="item-top">
      ...
      <div class="meta">Interest paid (est): <strong>${money(r.interestPaid)}</strong> • APR ${r.apr.toFixed(2)}% • Min ${money(r.minPay)}</div>
    `;
...
async function generatePDF(){
  ...
  doc.setFontSize(18);
  doc.text("Debt War Room — Printable Board", 48, y);
  y += 22;
  ...
  y += 16;
  doc.text(`Attack Power: ${money(attack)}   Total Debt: ${money(debtTotal)}   Assets: ${money(assetsTotal)}   Net Worth: ${money(assetsTotal - debtTotal)}`, 48, y);
  y += 22;
  ...
  doc.setFont("helvetica","bold");
  doc.setFontSize(12);
  doc.text("Freed Monthly Payments", 48, y);
  y += 14;

  doc.setFont("helvetica","normal");
  doc.setFontSize(10);
  if (!state.freed.length){
    doc.text("None logged yet.", 48, y);
  } else {
    state.freed.forEach(f=>{
      doc.text(`${f.month || ""} — ${f.name || "Freed Payment"}: ${money(f.amount || 0)}/mo`, 48, y);
      y += 14;
      if (y > 720){ doc.addPage(); y = 48; }
    });
  }

  doc.save("Debt_War_Room_Board.pdf");
}
...
function genFromLastPay(){
  ...
  if (!lastISO){ alert("Enter Last Pay Date"); return; }
...
function exportJSON(){
  ...
  a.href = url;
  a.download = `debt-war-room_${activeProfile}.json`;
  a.click();
...
function bindInputs(){
  ...
  $("#budgetGroceries").oninput = ()=>{state.settings.budgets.groceries = Number($("#budgetGroceries").value||0); saveState(); renderAll();};
  $("#budgetGas").oninput = ()=>{state.settings.budgets.gas = Number($("#budgetGas").value||0); saveState(); renderAll();};
  $("#budgetOther").oninput = ()=>{state.settings.budgets.other = Number($("#budgetOther").value||0); saveState(); renderAll();};
  $("#safetyFloor").oninput = ()=>{state.settings.safetyFloor = Number($("#safetyFloor").value||0); saveState(); renderAll();};
  $("#efGoal").oninput = ()=>{state.settings.efGoal = Number($("#efGoal").value||0); saveState(); renderAll();};

  const strat = $("#strategySelect");
  const extra = $("#extraPayment");
  if (strat){
    strat.value = state.settings.strategy || "hybrid";
    strat.onchange = ()=>{state.settings.strategy = strat.value; saveState(); renderAll();};
  }
  if (extra){
    extra.value = state.settings.extraPayment || 0;
    extra.oninput = ()=>{state.settings.extraPayment = Number(extra.value||0); saveState(); renderAll();};
  }
...
function renderAll(){
  ...
  else { const dl = $("#debtList"); if (dl) dl.innerHTML = ""; }

  if (state.settings.modes.invest) renderAssets();
  else { const al = $("#assetList"); if (al) al.innerHTML = ""; }
